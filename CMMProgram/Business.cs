...
public partial class CMMProgramUI:SnapEx.BaseUI
{
    ...
    void ComponentHelper(Action<ProbeData> action,string probeName) 
    {
    ...
    {
        RefreshUI();
        _probeDatas = new List<ProbeData>();
        ...
        if (File.Exists(_probeDataPath))
        {
            ...
            {
                _probeDatas = Newtonsoft.Json.JsonConvert.DeserializeObject<List<ProbeData>>(json);
                var memberStrings = new List<string>();
                _probeDatas.ForEach(u => {
                    memberStrings.Add(u.ProbeName);
                });
                enumSelectTool.SetEnumMembers(memberStrings.ToArray());
            }
        }
