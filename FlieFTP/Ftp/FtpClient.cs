...
namespace EACT
{
    public class FTPClient
    {
        ...
        public void Put(string strFolder, string strFileNameMask)
        {
            ...
            var DirName = dir.Name.ToString();
            //创建这个目录
            MkDir(DirName);
            //切换到这个目录
            ChDir(DirName);
            FileInfo[] strFiles = dir.GetFiles(strFileNameMask);
            foreach (FileInfo strFile in strFiles)
            {
                //strFile是完整的文件名(包含路径)
                Put(strFile.FullName);
            }
            DirectoryInfo[] allDir = dir.GetDirectories();
            foreach (DirectoryInfo d in allDir)
            {
                Put(d.FullName, strFileNameMask);
            }
            //切换回来原来目录
            ChDir("..");
        }
        ...
        /// 接收和发送数据的缓冲区
        /// </summary>
        private static int BLOCK_SIZE = 32768;
